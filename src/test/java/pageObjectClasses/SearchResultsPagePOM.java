...
public class SearchResultsPagePOM extends BasePageObject {
	...
	/*
	 * if (products.size() > 0) { // Generate a random index Random random = new
	 * Random(); int randomIndex = random.nextInt(products.size());
	 * 
	 * // Select the random product WebElement selectedProduct =
	 * products.get(randomIndex); System.out.println("Selected Product: " +
	 * selectedProduct.getText());
	 * 
	 * // Perform an action, e.g., click the product selectedProduct.click(); } else
